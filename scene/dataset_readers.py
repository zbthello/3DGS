...
def storePly(path, xyz, rgb):
    ...
    # 每个内建类型都有一个唯一定义它的字符代码：'f'：浮点,'u'：无符号整数
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    if not os.path.exists(ply_path):
        ...
        storePly(ply_path, xyz, rgb)  # 存储点云
    try:
        pcd = fetchPly(ply_path)  # 获取点云数据
    except:
        pcd = None

    """
        返回
        point_cloud：点云数据(xyz坐标，rgb颜色，n法向量【空】)
        train_cameras：训练集图片信息(每张图片信息包括uid=uid, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                              image_path=image_path, image_name=image_name, width=width, height=height)
        test_cameras：测试集图片信息
        nerf_normalization：train_cam_infos集的所有相机的平均中心的负值和所在球半径大小
        ply_path：点云文件位置
    """
